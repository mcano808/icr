...
public class DiscoverService
{
	...
	job.getJobProperties().add(new JobProperty("Params","Params Description", true, DataType.STRING, "Group A", null));	

	jobs.add(job);
	job = new IngestJob();
	job.setName("File Drop CSV");
	job.setDescription("A file drop box that is csv based data");
	job.getJobProperties().add(new JobProperty("Directory","Directory Description", true, DataType.STRING, "Group A", null));
	...
	jobs.add(job);
	job.getJobProperties().add(new JobProperty("Host","Host description", true, DataType.STRING, "Group A", "localhost"));
    ...
    {
	Resources<DataType> dataTypes = new Resources<>(Arrays.asList(
		DataType.values()));
